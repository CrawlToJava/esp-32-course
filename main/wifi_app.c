...
/*
Initiliaze the TCP stack and default WiFI configuration
*/

static void wifi_app_default_wifi_init(void)
{
    // Initiliaze the TCP stack
    ESP_ERROR_CHECK(esp_netif_init());

    // Initiliaze WiFi configuration - operations must be in this order!
...
/*
Configures the WiFi access point settings and assigns the static IP to the soft IP
*/
static void wifi_app_soft_ap_config(void)
{
    ...
    wifi_config_t ap_config = {
        .ap = {
            ...
            .ssid_hidden = WIFI_AP_SSID_HIDEN,
    ...
    // Comfigure DHCP for the AP
    esp_netif_ip_info_t ap_ip_info;
    memset(&ap_ip_info, 0x00, sizeof(ap_ip_info));

    esp_netif_dhcps_stop(esp_netif_ap);             // must call this first
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip); // Assign access point`s static IP, Netmask, Gateway
...
static void wifi_app_task(void *pvParameters)
{
    ...
    // Initiliaze the event handler
    wifi_app_event_handler_init();

    // Initiliaze the TCP/IP and wifi config
...
void wifi_app_start(void)
{
    ...
    // Disable default wifi loging messages
    esp_log_level_set("WIFI", ESP_LOG_NONE);

    // Create wifi message queue
